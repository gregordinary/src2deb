...
- `curl`, for components built from a `source.tarball` release archive or a
  `source.dsc` source package. Needed only to fetch what is not already cached,
  so a work directory that has the archives builds without it.
